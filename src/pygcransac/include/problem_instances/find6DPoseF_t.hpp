...
template<class Pose6DEstimatorF>
int find6DPoseF_(
	...
	int lo_number)
{
	...
	inlier_selector::EmptyInlierSelector<Pose6DEstimatorF, 
		AbstractNeighborhood> inlier_selector(neighborhood_graph.get());

	if (use_sprt)
	{
		// Initializing SPRT test
		preemption::SPRTPreemptiveVerfication<Pose6DEstimatorF> preemptive_verification(
			points,
			estimator,
			min_inlier_ratio_for_sprt);

		GCRANSAC<Pose6DEstimatorF,
			AbstractNeighborhood,
			MSACScoringFunction<Pose6DEstimatorF>,
			preemption::SPRTPreemptiveVerfication<Pose6DEstimatorF>> gcransac;
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 4; j++) {
			poseF[i * 4 + j] = (double)model.descriptor(i, j);
		}
	}

	poseF[12] = (double)model.descriptor(3, 3);
